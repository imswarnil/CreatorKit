...
pnpm install
pnpm build              # tokens → core → ui, in order
```

Three things to look at:

```bash
pnpm site:dev                         # http://localhost:8080  the site
pnpm ghost:start                      # http://localhost:2370  the Ghost theme
pnpm theme:dev                        # CSS watch + browser-sync for the theme
```

Ports here: **8080** the site, **2370** Ghost. `imswarnil.com` keeps 2368 and is untouched by
...
├─ _legacy/         the previous design system, whole — and what
│                   creator.imswarnil.com serves. See _legacy/README.md
├─ scripts/         stage-site.mjs — _legacy/docs → _site, as pages.yml did
└─ docs/
...
**4. Document it.** The class list and the inventory are generated by `tools/css-inventory`
into the package's `INVENTORY.md`. What you write is the part no tool can produce: what it
...
`creator.imswarnil.com` serves the **previous** design system's own documentation site —
the one published from `Swarnil-Design-System` at `f84ccf7` — not a React app. There is
no Next.js in this repo any more.

```bash
pnpm site:build    # _legacy CSS (PostCSS) + docs/_build/build.py → _site
pnpm site:dev      # that build on http://localhost:8080
```

- **The pages are generated by Python.** `_legacy/docs/_build/build.py` owns the nav, the
  ordering, the per-page TOC and prev/next. Authored content lives in the `content_*.py`
  modules beside it; extracted demos live in `fragments/`.
- **The domain lives in one constant.** `_legacy/docs/_build/build.py:21` — `SITE`. The
  `CNAME`, `sitemap.xml`, `robots.txt` and every canonical URL are generated from it.
- **`_site` is derived.** `scripts/stage-site.mjs` copies `_legacy/docs` into it, drops
  `_build` (the generator, not output) and writes `.nojekyll` — the four lines that
  commit's `pages.yml` ran. It is gitignored; never edit it.

### Deploying

```bash
pnpm cf:preview    # serve the staged site locally on workerd
pnpm cf:deploy     # build and ship to Cloudflare
```

Static assets, no Worker script — `wrangler.jsonc` has no `main`. `html_handling` is
...
| `tools/` | recipe-to-css, props-gen, css-inventory, icon-build |
| The site | `_legacy` rebuilt and served whole — 212 pages, as published at `f84ccf7` |
